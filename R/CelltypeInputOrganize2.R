...
#' @export
CelltypeInputOrganize2 = function(Tissue_whole_peptide, celltype2, Modalities, address_celltype_FC_P_0){
...
for (m in 1:length(celltype)) {# this section to test use more celltype 
  ...
  qureytable<-data.frame(Protein.Id=Tissue_whole_peptide$Protein.Id, Gene.Symbol=Tissue_whole_peptide$Protein.Name, PeptideSeq=Tissue_whole_peptide$Sequence,Tissue_whole_peptide[,celltype_one_list],Plasma_FC=df_Alb[,celltypeFC[m]],Plasma_p=df_Alb[,celltypepvalue[m]])
  ...
  write.csv(Peptide_proteoform,file=address_proteoform,row.names=FALSE)
  write.csv(Peptide_proteoform_list,file=address_proteoform_list,row.names=FALSE)
